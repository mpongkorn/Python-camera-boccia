...
import threading
# from inference_sdk import InferenceHTTPClient # Kept as per user's note
# from ultralytics import YOLO # Kept as per user's note

# --------------- PLC Communication (Example) ---------------------
# lib for connecting and editing PLC with Ethernet SLMP
import pymcprotocol # This line is here to prevent NameError if PLC code is uncommented
# -----------------------------------------------------------------
...
class FieldMeasureApp:
    ...
    def __init__(self, root_tk):
        ...
        # StringVars for Target Ball 2 coordinates
        self.target_x2_cm_str = StringVar(value="300.0") # Default X for Ball 2
        self.target_y2_cm_str = StringVar(value="750.0") # Default Y for Ball 2
    ...
    def create_widgets(self):
        ...
        self.control_frame = Frame(main_frame, width=320) # Slightly wider for new inputs
        ...
        # MODIFICATION: Target Ball 2 Position Section
    ...
    def detect_balls_in_frame(self):
        ...
        color_ranges = {
            'white': [(np.array([0, 0, 160]), np.array([180, 55, 255]))],
        ...
        for color_name, hsv_ranges_list in color_ranges.items():
            ...
            for cnt in contours_hsv:
                ...
                if 200 < area < 50000:
                    ...
                    if radius > current_min_radius_for_primary * 0.8 and circularity > current_circularity_thresh_for_primary * 0.8:
    ...
    def process_measurements(self):
        ...
        # MODIFICATION: Get Target Ball 2 coordinates from user input
        try:
            x2_cm_str = self.target_x2_cm_str.get()
            y2_cm_str = self.target_y2_cm_str.get()
            x2_cm = float(x2_cm_str)
            y2_cm = float(y2_cm_str)
            print(f"Info: Using Target Robot position: X={x2_cm} cm, Y={y2_cm} cm")
        except ValueError:
            print(f"Warning: Invalid input for Target Robot coordinates ('{x2_cm_str}', '{y2_cm_str}'). Using default 0.0, 0.0.")
            x2_cm = 0.0 # Default fallback
            y2_cm = 0.0 # Default fallback
            self.target_x2_cm_str.set(str(x2_cm)) # Update GUI to show fallback
        ...
        cv2.circle(warped_img_native_cm, (int(x1_cm), int(y1_cm)), 6, (0, 0, 255), -1) # Ball 1 in red
        cv2.putText(warped_img_native_cm, "B1", (int(x1_cm)+5, int(y1_cm)-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255),1)
        cv2.circle(warped_img_native_cm, (int(x2_cm), int(y2_cm)), 6, (0, 255, 0), -1) # Ball 2 in green
        ...
        # Displaying coordinates as requested
        Label(results_frame, text=f"Ball 1 - Original Image Coords (pixels): {self.ball_pt1}", font=('Arial', 10)).pack(anchor=W)
        Label(results_frame, text=f"Ball 1 - Field Coords (cm, from Top-Left): ({x1_cm:.1f}, {y1_cm:.1f})", font=('Arial', 10, 'bold')).pack(anchor=W) # Highlighted
        Label(results_frame, text=f"Target Robot - Field Coords (cm, from Top-Left): ({x2_cm:.1f}, {y2_cm:.1f})", font=('Arial', 10)).pack(anchor=W)
        ttk.Separator(results_frame, orient='horizontal').pack(fill='x', pady=5)
        Label(results_frame, text=f"Distance B1 to Robot: {distance_cm:.1f} cm", font=('Arial', 12, 'bold')).pack(anchor=W)
        Label(results_frame, text=f"Angle B1 to Robot (from B1, rel. to Y-axis): {angle_degrees:.1f}°", font=('Arial', 12, 'bold')).pack(anchor=W)
